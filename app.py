...
openai.api_key = "sk-vY950un7WR4qcQhpxgnqJjRGo3_g22FF7mjVe1yh7OT3BlbkFJLS_EbKN0THu8GXRUA7OLB61yy5ibzdNmgDGiwmCDoA"
...
def chat():
    ...
    try:
        # Request a response from OpenAI's API
...
def get_products():
    ...
    # Modified query to order by v.status (assuming 'active' and 'inactive' are the possible values)
    query = """
        SELECT 
            p.*, 
            v.store_name, 
...
def vendor_signup():
    try:
        ...
        # Check for missing fields
        if not all([vendor_name, email, password, store_name, address, city]):
            return jsonify({'error': 'All required fields (vendor name, email, password, store name, address, city) must be filled!'}), 400
        ...
        cursor.execute(
            "INSERT INTO vendor (vendor_name, email, password, store_name, address, phone_number, status ,city) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s, 'non active')",
            (vendor_name, email, password, store_name, address, phone_number, city)
        )
        conn.commit()
